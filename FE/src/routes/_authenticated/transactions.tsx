    ...
    return (
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            <div className="space-y-1">
                ...
                <p className="text-xs text-slate-500">Nơi tra cứu và lọc tất cả các biến động tài chính lịch sử</p>
            </div>
                    ...
                    <table className="w-full text-left text-sm border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredTxs.length === 0 ? (
                                <tr>
                                    <td colSpan={3} className="text-center p-8 text-slate-400 text-xs">Không tìm thấy dữ liệu phù hợp.</td>
                                </tr>
                            ) : (
                                filteredTxs.map(t => (
                                    <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                                        ...
                                        <td className="p-4">
                                            <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-semibold ${t.type === 'expense' ? 'bg-rose-50 text-rose-700' : 'bg-emerald-50 text-emerald-700'}`}>
                                                {t.type === 'expense' ? 'Chi tiêu' : 'Thu nhập'}
                                            </span>
                                        </td>
                                        <td className={`p-4 text-right font-bold ${t.type === 'expense' ? 'text-rose-600' : 'text-emerald-600'}`}>
