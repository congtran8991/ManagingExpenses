        ...
        <div className="p-6 max-w-7xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <Card className="bg-white border shadow-xs">
                    <CardHeader><CardTitle className="text-sm font-bold text-slate-500">Số lượng tài khoản ví</CardTitle></CardHeader>
                    <CardContent><p className="text-2xl font-bold text-slate-800">{wallets.length} Ví hoạt động</p></CardContent>
                </Card>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <Card className="lg:col-span-1">
                    <CardHeader><CardTitle className="text-base font-bold">Số dư tài khoản ví</CardTitle></CardHeader>
                ...
                <Card className="lg:col-span-2">
                    ...
                    <CardContent className="space-y-3">
                        {recentTxs.length === 0 ? (
                            <p className="text-xs text-slate-400 text-center py-4">Chưa có giao dịch nào được ghi nhận.</p>
                        ) : (
                            recentTxs.map(t => (
                                <div key={t.id} className="flex justify-between items-center border-b pb-2 text-sm last:border-0 last:pb-0">
                                    <div>
                                        <p className="font-semibold text-slate-800">{t.note}</p>
                                        <p className="text-xs text-slate-400">ID Ví: {t.wallet_id}</p>
                                    </div>
                                    <span className={t.type === 'expense' ? 'text-red-500 font-bold' : 'text-emerald-600 font-bold'}>
